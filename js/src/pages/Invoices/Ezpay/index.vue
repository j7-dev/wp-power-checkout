...
import { TFormData } from '@/pages/Invoices/Ezpay/Shared/types'

const gatewayId = 'ezpay'
...
<template>
	...
	>
		...
		>
			<template #label>
				<span class="flex gap-x-2 items-center">
					<span>啟用測試模式</span>
					<el-tooltip
						content="開發人員專用，啟用後將使用藍新 ezPay 測試帳號測試開立"
						placement="top"
					>
						<el-icon><InfoFilled /></el-icon>
					</el-tooltip>
		...
		<el-alert
			v-if="!isTestMode"
			title="正式模式請填入藍新 ezPay 商家後台的商店代號、HashKey、HashIV，否則無法開立發票"
			type="info"
			class="mb-4"
			:closable="false"
			show-icon
		...
		<el-alert
			:title="
				isTestMode
					? '測試環境發票 API：https://cinv.ezpay.com.tw'
					: '正式環境發票 API：https://inv.ezpay.com.tw'
			"
			type="warning"
			class="mb-4"
			:closable="false"
		...
		<el-form-item :required="!isTestMode" prop="merchant_id">
			<template #label>
				<span class="flex gap-x-2 items-center">
					...
					<el-tooltip
						content="藍新 ezPay 電子發票商店代號（與金流可能不同）"
						placement="top"
					>
						<el-icon><InfoFilled /></el-icon>
					</el-tooltip>
		...
		<el-form-item :required="!isTestMode" prop="hash_key">
			<template #label>
				<span class="flex gap-x-2 items-center">
					<span>HashKey</span>
					<el-tooltip content="藍新 ezPay HashKey（32 bytes）" placement="top">
						<el-icon><InfoFilled /></el-icon>
					</el-tooltip>
				</span>
			</template>
		...
		<el-form-item :required="!isTestMode" prop="hash_iv">
			<template #label>
				<span class="flex gap-x-2 items-center">
					<span>HashIV</span>
					<el-tooltip content="藍新 ezPay HashIV（16 bytes）" placement="top">
						<el-icon><InfoFilled /></el-icon>
					</el-tooltip>
				</span>
			</template>
