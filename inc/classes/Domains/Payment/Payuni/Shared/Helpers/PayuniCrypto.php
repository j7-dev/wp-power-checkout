<?php
/**
 * PAYUNi 統一金流（UPP V2）AES-256-GCM 加解密 + SHA256 HashInfo
 *
 * ⚠️ 與綠界（AES-128-CBC）截然不同：PAYUNi 金流與物流共用 AES-256-GCM。
 *
 * 與 J7\PowerCheckout\Domains\Logistics\Payuni\Shared\Helpers\PayuniCrypto 同源：
 *   金流（Payment）與物流（Logistics）共用 PAYUNi AES-256-GCM 演算法，演算法 100% 一致。
 *   為避免「Payment 反向依賴 Logistics」與「改動既有 Logistics code」的耦合風險，
 *   此處在 Payment namespace 自建一份同源 copy。
 *   未來若有第三個 PAYUNi 領域，可考慮提升至 Shared 共用層集中維護。
 *   ⚠️ 兩版演算法不得分歧——PayuniCryptoGoldenVectorTest 以等價性斷言鎖死兩者輸出一致。
 *
 * 加密流程（payuni-upp-v2 encryption.md）：
 ...
 *
 * @see .claude/skills/payuni-upp-v2/references/encryption.md
 */
...
/** PAYUNi UPP V2 AES-256-GCM 加解密 + SHA256 HashInfo Helper */
final class PayuniCrypto {
	...
	public function __construct( string $hash_key, string $hash_iv ) {
		// PAYUNi 後台複製偶有 trailing 空白，一律 trim（payuni-upp-v2 §金鑰規範）
	 ...
	 *
	 * 空值（'' / null）欄位過濾後再 http_build_query（對齊官方 PHP 範例）。
	 *
	 * @param array<string, mixed> $data 要加密的明文陣列（含 MerID / Timestamp 等）
	 *
	 ...
	 *
	 * 順序固定 HashKey + EncryptInfo + HashIV，不可換序（payuni-upp-v2 §HashInfo）。
	 *
	 * @param string $encrypt_info EncryptInfo（hex 字串）
	 *
